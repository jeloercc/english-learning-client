      ...
      {items.map((item) => {
        ...
        return (
          <div key={item.label} className="flex items-center gap-1.5 sm:gap-2">
            <span className="font-mono text-xs text-zinc-400 w-12 sm:w-14">{item.label}</span>
            <div className="w-14 sm:w-20 h-1.5 bg-zinc-200">
...
interface SidebarContentProps {
  ...
  setActiveLevel: (l: CEFRLevel) => void;
  activeSection: Section;
  setActiveSection: (s: Section) => void;
  currentLevel: typeof LEVELS[number];
  onNavigate?: () => void; // close drawer on mobile after selection
}
    ...
    <>
      ...
      <div className="p-4 border-t border-zinc-100">
        <p className="font-mono text-xs text-zinc-400 leading-relaxed">
          Free tier · Progress saves locally
        </p>
      </div>
    </>
  );
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F4F0] text-zinc-900" style={{ fontFamily: "system-ui, sans-serif" }}>
      {/* Top bar */}
      <header className="border-b border-zinc-300 bg-white px-4 md:px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {/* Level pill */}
          <div className="flex items-center gap-1.5 border border-zinc-200 bg-white px-2 sm:px-3 py-1">
      ...
      <div className="flex min-h-[calc(100vh-49px)]">
        ...
        <nav className="hidden md:flex w-56 shrink-0 bg-white border-r border-zinc-300 flex-col">
        ...
        <main className="flex-1 overflow-auto">
          {/* Content header */}
          <div className="border-b border-zinc-200 bg-white px-4 md:px-8 py-4">
